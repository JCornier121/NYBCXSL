...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    version="2.0">
    <xsl:template match="/">
<xsl:variable name="Dir" select="document('http://incapaptivated.com/XMLfiles/userdir.xml')/Directory/EE"/>
        ...
        <xsl:for-each-group select="root/row" group-by="Username">
            <xsl:result-document href="/c:/{$dateNow2}/{$Dir[@id=(current()/Username)]/@Department}/{$Dir[@id=(current()/Username)]/@id}FOX6.1.fo" xml:space="preserve">
...
<fo:table-row text-indent="5pt" border-left-width="2pt" border-left-color="black" border-left-style="solid" border-right-width="2pt" border-right-color="black" border-right-style="solid">
                        ...
                        <fo:table-row text-indent="3pt" border-left-width="2pt" border-left-color="black" border-left-style="solid" border-right-width="2pt" border-right-color="black" border-right-style="solid">
                            ...
                            <fo:table-cell>
                                <fo:block text-align="start" font-size="9pt"><xsl:value-of select="$Dir[@id=(current()/Username)]/@Supervisor_Name"/>
                                </fo:block>
                            </fo:table-cell>
                        </fo:table-row>
     ...
     <xsl:for-each select="current-group()">
                        <fo:table-row border-width=".5pt" border-style="solid" border-color="black">
                            <fo:table-cell>
                                ...
                                <fo:block keep-together.within-line="always" text-align="left" font-size="10pt" text-indent="4.5pt"><xsl:value-of select="Mapped_Item_Name"></xsl:value-of></fo:block></fo:block-container>
                            </fo:table-cell>
                            <fo:table-cell end-indent="1pt">
                                <fo:block text-align="right" font-size="10pt"><xsl:value-of select="substring(Mapped_Item_Completed_Date, 1, 10)"/></fo:block>
                            </fo:table-cell>
                        </fo:table-row>
                                                </xsl:for-each>
...
<fo:block text-align="start" vertical-align="top" font-size="10pt">Assessment Review was completed:</fo:block>
                        ...
                        <fo:table-row height=".65in" border-width="1pt" border-style="solid" border-spacing="8pt" border-color="black">
                            ...
                            <fo:table-cell>
                                <fo:block text-align="start" font-size="10pt" vertical-align="top">Date:</fo:block></fo:table-cell>
                            </fo:table-row>
